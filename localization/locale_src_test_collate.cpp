...
void test_comp(std::locale l,std::basic_string<Char> left,std::basic_string<Char> right,int ilevel,int expected)
{
    ...
    cppcms::locale::collator_base::level_type level = static_cast<cppcms::locale::collator_base::level_type>(ilevel);
    TEST(cppcms::locale::comparator<Char>(l,level)(left,right) == (expected < 0));
    if(ilevel==0) {
    ...
    cppcms::locale::collator<Char> const &coll=std::use_facet<cppcms::locale::collator<Char> >(l);
