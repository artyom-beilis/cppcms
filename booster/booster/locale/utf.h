...
//
#ifndef BOOSTER_LOCALE_UTF_H_INCLUDED
#define BOOSTER_LOCALE_UTF_H_INCLUDED
...
namespace utf {
    /// \cond INTERNAL
    #ifdef __GNUC__
    #   define BOOSTER_LOCALE_LIKELY(x)   __builtin_expect((x),1)
    #   define BOOSTER_LOCALE_UNLIKELY(x) __builtin_expect((x),0)
    #else
    #   define BOOSTER_LOCALE_LIKELY(x)   (x)
    #   define BOOSTER_LOCALE_UNLIKELY(x) (x)
    #endif
    /// \endcond
    ...
    struct utf_traits<CharType,1> {
        ...
        static int trail_length(char_type ci) 
        {
            ...
            if(c < 128)
                return 0;
            if(BOOSTER_LOCALE_UNLIKELY(c < 194))
                return -1;
            if(c < 224)
                return 1;
            if(c < 240)
                return 2;
            if(BOOSTER_LOCALE_LIKELY(c <=244))
                return 3;
            return -1;
        }
    ...
    struct utf_traits<CharType,4> {
        ...
        {
            if(BOOSTER_LOCALE_UNLIKELY(current == last))
                return booster::locale::utf::incomplete;
            code_point c=*current++;
            if(BOOSTER_LOCALE_UNLIKELY(!is_valid_codepoint(c)))
                return booster::locale::utf::illegal;
            return c;
        }
