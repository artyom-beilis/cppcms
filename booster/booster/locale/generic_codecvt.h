...
//
#ifndef BOOSTER_LOCALE_GENERIC_CODECVT_HPP
#define BOOSTER_LOCALE_GENERIC_CODECVT_HPP
...
#define BOOSTER_LOCALE_DO_LENGTH_MBSTATE_CONST
#endif
...
protected:
    ...
    {
        booster::uint16_t &state = *reinterpret_cast<booster::uint16_t *>(&s);
#ifdef DEBUG_CODECVT            
    ...
    #ifdef BOOSTER_LOCALE_DO_LENGTH_MBSTATE_CONST
            const   
    #endif
            &std_state,
            char const *from,
            ...
            size_t max) const
    {
        ...
        booster::uint16_t &state = *reinterpret_cast<booster::uint16_t *>(&std_state);
        #else
        size_t save_max = max;
        booster::uint16_t state = *reinterpret_cast<booster::uint16_t const *>(&std_state);
        #endif
        ...
        while(max > 0 && from < from_end){
            ...
            booster::uint32_t ch=implementation().to_unicode(cvt_state,from,from_end);
            if(ch==booster::locale::utf::incomplete || ch==booster::locale::utf::illegal) {
                from = prev_from;
                break;
            }
            max --;
            ...
            uchar *&to_next) const
    {
        ...
        while(to < to_end && from < from_end)
        {
            ...
            if(ch==booster::locale::utf::illegal) {
                from = from_saved;
                r=std::codecvt_base::error;
                break;
            }
            if(ch==booster::locale::utf::incomplete) {
                from = from_saved;
            ...
            if(ch <= 0xFFFF) {
                *to++=ch;
                ...
                ch-=0x10000;
                booster::uint16_t vh = ch >> 10;
                booster::uint16_t vl = ch & 0x3FF;
                booster::uint16_t w1 = vh + 0xD800;
                booster::uint16_t w2 = vl + 0xDC00;
                if(state == 0) {
                    from = from_saved;
                    *to++ = w1;
                    state = 1;
            ...
            char *&to_next) const
    {
        ...
        std::cout << "Left in " << std::dec << from_end - from << " out " << to_end -to << std::endl;
            ...
            if(state != 0) {
                ...
                // second surrogate
                booster::uint16_t w1 = state;
                booster::uint16_t w2 = *from; 
                // we don't forward from as writing may fail to incomplete or
                // partial conversion
                if(0xDC00 <= w2 && w2<=0xDFFF) {
                    booster::uint16_t vh = w1 - 0xD800;
                    booster::uint16_t vl = w2 - 0xDC00;
