...
bool test_to(booster::locale::util::base_converter &cvt,char const *s,unsigned codepoint)
{
    size_t len = strlen(s);
...
void test_shiftjis(booster::locale::util::base_converter* pcvt)
{
    booster::locale::hold_ptr<booster::locale::util::base_converter> cvt(pcvt);
...
int main()
{
    try {
        ...
        cvt.reset(booster::locale::impl_icu::create_uconv_converter("Shift-JIS"));
        TEST(cvt.get());
        ...
        cvt.reset(booster::locale::impl_posix::create_iconv_converter("Shift-JIS"));
        if(cvt.get()) {
            test_shiftjis(cvt.release());
        }
        else {
