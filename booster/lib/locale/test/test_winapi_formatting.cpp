...
#ifdef BOOSTER_LOCALE_NO_WINAPI_BACKEND
#include <iostream>
int main()
{
...
{
    bool res = s1 == booster::locale::conv::from_utf(s2,"UTF-8");
    #ifdef DEBUG_FMT
    if(!res)
        std::cout << "[" << s1 << "]!=["<<booster::locale::conv::from_utf(s2,"UTF-8")<<"]"<<std::endl;
    #endif
    return res;
}
...
bool equal(std::wstring const &s1,std::wstring const &s2)
{
    ...
    #ifdef DEBUG_FMT
    if(!res)
        std::cout << "[" << booster::locale::conv::from_utf(s1,"UTF-8") << "]!=["<<booster::locale::conv::from_utf(s2,"UTF-8")<<"]"<<std::endl;
    #endif
    return res;
}
...
{
    bool res = s1 == booster::locale::conv::to_utf<wchar_t>(s2,"UTF-8");
    #ifdef DEBUG_FMT
    if(!res)
        std::cout << "[" << booster::locale::conv::from_utf(s1,"UTF-8") << "]!=["<<s2<<"]"<<std::endl;
    #endif
    return res;
