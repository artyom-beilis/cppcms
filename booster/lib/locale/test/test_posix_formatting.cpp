...
//
#ifdef BOOSTER_LOCALE_NO_POSIX_BACKEND
#include <iostream>
int main()
{
...
{
    return s1 == booster::locale::conv::to_utf<wchar_t>(s2,nl_langinfo_l(CODESET,lc));
}
...
void test_by_char(std::locale const &l,locale_t lreal)
{
...

    {
        ...
        if(std::use_facet<booster::locale::info>(l).country()=="US")
...

    {
        ...
        #ifdef DEBUG_FMT
        std::cout << "[" << booster::locale::conv::from_utf(ss.str(),"UTF-8") << "]=\n" ;
        std::cout << "[" << booster::locale::conv::from_utf(buf,"UTF-8") << "]\n" ;
        #endif
